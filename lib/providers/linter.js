...
const CTX_VAR = /((%[a-zA-Z0-9-_]+%)|(%%[a-zA-Z0-9-_]+%%))/g;
...
function fakeCtxVars(src) {
  var match = CTX_VAR.exec(src);
  while (match) {
    src = src.replace(new RegExp(match[1], 'g'), shortid());
    match = CTX_VAR.exec(src);
  }
  return src;
}
...
module.exports = {
  ...
  lint: (function(_this) {
    return function(textEditor) {
      ...
      text = fakeCtxVars(textEditor.getText());
